...
//		int y=position[1]+random.nextInt(position[2]-position[1]);
	...
	public boolean isAttack(MonsterSprite monsterSprite)
	{
		int x= monsterSprite.getX();
				   ...
				   monsterSprite.getState()==ZombieSprite.CREEP)
				{
							...
							&& (x + speed) >= GameConsts.MONSTER_DOOR_COLLISION[doorSet])
					{
						if (!this.isLayer(layer, doorSet))
						{
							...
							int dy = x + speed
									- GameConsts.MONSTER_DOOR_COLLISION[doorSet];
						...
						{
							monsterSprite.setState(VampireSprite.ATTACK);
						}
					}
					if (isDoor)
					{
					...
					if (this.isAttack(monsterSprite))
					{
	...
	private boolean isLayer(int layer, int doorSet)
	{
		...
		else if (doors[doorSetArray[0]].getState() == DoorSprite.DOWN
				&& layer == 1)
		{
			clearance = false;
		} 
		...
		else if (doors[doorSetArray[1]].getState() == DoorSprite.DOWN
				&& layer == 2)
		{
			clearance = false;
		}
	...
	public void stoneCollisionMonster(int layer)
	{
		for(int i=0;i<monsters.size();i++)
		{
			...
			if(monsterSprite.getLayer()==layer)
			{
				...
				{
					int hp=monsterSprite.getHp();
					hp=hp-GameConsts.STONE_ATTACK;
					if(hp<=0)
					{
						hp=0;
						monsterSprite.setState(MonsterSprite.DEAD);
						this.addAirSprite(monsterSprite,layer);
					}
					else
					{
						monsterSprite.setHurt(true);
	...
	public void update()
	{
		...
		if(subState==PLAY)
		{
			...
			for(int i=0;i<heros.length;i++)
			{
				...
				else if(heroSprite.getState()==HeroSprite.STONE_END)
				{
			...
			for(int i=0;i<attackType.length;i++)
			{
				...
				if(attackType[i]==ATTACK_NONE)
				{
					if(heroSprite.getState()==HeroSprite.SHOOT||
					   heroSprite.getState()==HeroSprite.STONE)
					{
				...
				else if(attackType[i]==ATTACK_STONE)
				{
					...
					{
						heroSprite.setState(HeroSprite.STONE);
					}
				}
			}
...
//			if(timeSprite.isBorn())
//			{
//				int[] layer=this.getRandomLayer(1);
//				for(int i=0;i<layer.length;i++)
//				{
//					this.addMonsterSprite(layer[i],random.nextInt(4));
//				}
//			}
		}
	...
	public void drawView(Canvas canvas)
	{
			...
			subState==PAUSE)
		{
			...
			this.drawSprite(canvas, arrows, -1);
			barSprite.drawView(canvas);
			menuSprite.drawView(canvas);
			//timeSprite.drawView(canvas);
		}
	}
			...
			if(event.getAction() == MotionEvent.ACTION_DOWN)
			{
				if(barSprite.isCollision(x,y))
				{
					if(barSprite.isPause())
					{
						...
						menuSprite.setState(UiSprite.DISABLE);
						subState=PLAY;
					}
				}
			}
